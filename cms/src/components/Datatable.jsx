...
export const DataTable = ({data, searchable = [], sortable = []}) => {
    ...
    useEffect(() => {
        let links = [<Pagination.Prev disabled={currentPage == 1} onClick={() => 
        ...
        links.push(<Pagination.Prev disabled={currentPage == totalPages} onClick={() => 
            setCurrentPage(currentPage + 1 )} />)
    ...
    return <Row className="align-items-end"> 
        <Col sm="auto" className="mb-3">
            ...
            <Form.Select value={perPage} onChange={ev => setPerPage(parseInt(ev.target
                .value))}>
        ...
        {searchable.length ? <Col sm="4" className="ms-auto mb-3">
            <Form.Control type="search" placeholder="Search" onChange={ev => setTerm(ev.
                    target.value)} />
            </Col> : null}
            <Col xs="12">
                {items.length ?
                            ...
                            {Object.keys(items[0]).map((k, i) =>{
                                if(sortable.includes(k)){
                                    return <th key={i} className="sortable" onClick={() =>{
                                        ...
                                        setSortBy(k)
                                    }}>{k}{sortBy == k ? <i className={`fa-solid fa-${direction == 'asc' ? 
